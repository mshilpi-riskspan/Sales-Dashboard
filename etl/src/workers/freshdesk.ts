...
async function run(env: Env): Promise<void> {
  ...
  const results = await syncFreshdesk(sf, env);
...
export default {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    return new Response(JSON.stringify({ ok: true, worker: 'riskspan-etl-freshdesk' }), { headers: { 'Content-Type': 'application/json' } });
  },
};
