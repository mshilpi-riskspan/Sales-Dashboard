...
async function run(env: Env): Promise<void> {
  ...
  const results = await syncSalesforce(sf, env);
...
export default {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    return new Response(JSON.stringify({ ok: true, worker: 'riskspan-etl-salesforce' }), { headers: { 'Content-Type': 'application/json' } });
  },
};
